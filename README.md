...
- 🌓 **自适应暗色模式**：页面自动适配系统暗色模式，覆盖所有组件（卡片、弹窗、侧边栏、标签、搜索框等）
- ⚡ **并发数可配置**：「批量重新分析」模态框中新增并发数量选择器（1-10），用户可自行控制速度与资源占用
- 🧹 **孤立标签一键清理**：「其他」下拉菜单新增「清理孤立标签」功能，自动删除无表情包使用的标签配置
- ⏱️ **批量分析超时与重试优化**：LLM 请求超时调整为 120 秒，支持最多 3 次重试（指数退避），取消后不再浪费 token
- 🐛 **修复 CSS 变量缺失**：新增 `--bg-secondary`、`--bg-input` 变量定义，避免暗色模式下的浏览器默认白色回退
