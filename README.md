...
2. 如果需要修改表情包以及对应的llm提示, 查看astrbot/data/memes_data目录下的结构, `memes_data.json`即对应的文件夹分类和描述的映射关系, `memes`目录下的文件夹名称即分类名称, 里面的图片即对应类别的表情包, 你可以新建一个类别的文件夹, 并在json文件中添加对应的描述来添加新的表情类别, 也可以直接往自带的表情包中添加新的表情(上传文件到对应目录即可)
