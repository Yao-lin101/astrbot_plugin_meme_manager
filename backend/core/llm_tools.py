...
async def send_meme(
    ...
    try:
        ...
        return (
            f"表情包已成功发送！已选择{desc_info}的表情包。"
            # "（提示：如果该表情包已能完全表达你的意思，或者你在调用工具前已经生成了回复文本，"
            # "请在此直接结束本次回复，不要复述或重复你已经说过的话。）"
        )
...
async def steal_meme(
    ...
    """保存并收录图片/表情包到当前人格的表情包库中。

    下载或解构图片完成后，交由 meme_stealer.process_stolen_image 走统一的去重/偏好/分类/入库流程。
    """
